    ...
    public class Binary {
        ...
        public static void StripSignatures(string filename) {
            ...
            using (var f = File.Open(filename, FileMode.Open)) {
                ...
                if (errCount != 0) {
                    throw new ClrPlusException("Had errors removing {0} certificates from file {1}".format(errCount, filename));
                }
            }
        }
            ...
            lock (this) {
                ...
                _saving = Task<Binary>.Factory.StartNew(() => {
                                    ...
                                    foreach (var ar in _mutableAssembly.AssemblyReferences) {
                                        ...
                                        if (!ar.PublicKeyToken.Any()) {
                                            ...
                                            if (dep == null) {
                                                ...
                                                throw new ClrPlusException("dependent assembly '{0}-{1}' not available for strong naming".format(ar.Name.Value,
                                                    ar.Version.ToString()));
                                            }
                                            ...
                                            if (dep._mutableAssembly.PublicKey.IsNullOrEmpty()) {
                                                if (!_loadOptions.HasFlag(BinaryLoadOptions.NoUnsignedManagedDependencies)) {
                                                    ...
                                                    throw new ClrPlusException("dependent assembly '{0}-{1}' not strong named".format(ar.Name.Value,
                                                        ar.Version.ToString()));
                                                }
                                            }
                    ...
                    if (!_loadOptions.HasFlag(BinaryLoadOptions.NoManifest) && Manifest.Value != null && Manifest.Value.Modified) {
                        ...
                        var manifestResource = new ManifestResource(IsLibrary ? ManifestType.IsolationAware : ManifestType.CreateProcess) {
                            ManifestText = Manifest.Value.ToString(),
                            Language = 1033
                        };
