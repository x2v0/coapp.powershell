...
namespace ClrPlus.Windows.PeBinary.Utility {
    ...
    /// <summary>
    ///     This class loads a certificate, and can persist knowledge of the certificate (ie, the path and password in encrypted form) but NEVER stores or moves the certificate itself.
    /// </summary>
    public class CertificateReference {
        ...
        public static CertificateReference Default {
            get {
                if (_default == null) {
        ...
        /// <summary>
        ///     Opens a certificate from a path or store. Stores are expressed as storeLocation:storeName\thumbprint or storeLocation:storeName\subjectName (will throw if not unique) where storeLocation must be either 'currentuser' or 'localmachine'
        /// </summary>
        ...
        public CertificateReference(string location, string password) {
            ...
            if (File.Exists(location)) {
                ...
                if (!_certificate.HasPrivateKey) {
                    throw new ClrPlusException("Certificate '{0}' does not have private key".format(_location));
                }
            }
        ...
        /// <summary>
        ///     Opens a certificate from a path or store. Stores are expressed as storeLocation:storeName\thumbprint or storeLocation:storeName\subjectName (will throw if not unique) where storeLocation must be either 'currentuser' or 'localmachine' If the certificate is a file, it will use the password stored in the encrypted registry.
        /// </summary>
        /// <param name="location"> </param>
        public CertificateReference(string location) {
            ...
            if (File.Exists(location)) {
                ...
                if (_certificate == null) {
                    throw new ClrPlusException("Unable to load certificate '{0}' with a private key".format(_location));
                }
            }
