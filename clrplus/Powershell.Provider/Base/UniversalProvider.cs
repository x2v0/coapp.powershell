...
namespace ClrPlus.Powershell.Provider.Base {
    ...
    /// </summary>
    public class UniversalProvider<T> :
        NavigationCmdletProvider,
        IPropertyCmdletProvider,
        IContentCmdletProvider,
        IDynamicPropertyCmdletProvider,
        ISecurityDescriptorCmdletProvider where T : UniversalProviderInfo {
        /* static UniversalProvider() {
            var file = Assembly.GetExecutingAssembly().ExtractFileResourceToTemp("UniversalProvider.format.ps1xml");
            new SessionState().InvokeCommand.InvokeScript("Update-FormatData -PrependPath {0}".format(file));
        } */

        protected T UniversalProviderInfo {
            get {
                return ProviderInfo as T;
            }
        }
        ...
        ///     <see
        ///         cref="System.Management.Automation.PseudoParameterDictionary" />
        ...
        protected override object NewDriveDynamicParameters() {
            // Console.WriteLine("==NewDriveDynamicParameters==");
        ...
        ///     <see
        ///         cref="System.Management.Automation.Provider.CmdletProvider.WriteError(ErrorRecord)" />
        ...
        protected XDictionary<string, ILocation> _locationCache;
        protected static XDictionary<Tuple<string, string>, string> _makePathCache = new XDictionary<Tuple<string, string>, string>();
        protected static XDictionary<Tuple<string, string>, string> _getParentPathCache = new XDictionary<Tuple<string, string>, string>();
        protected static XDictionary<string, string> _getChildNameCache = new XDictionary<string, string>();

        protected XDictionary<string, ILocation> LocationCache {
            get {
                return _locationCache ?? (_locationCache = new XDictionary<string, ILocation>());
            }
        }

        protected ILocation GetLocation(string path) {
            return LocationCache.GetOrAdd(path, () => UniversalProviderInfo.GetLocation(path));
        }
        ...
        protected override object ItemExistsDynamicParameters(string path) {
            // Console.WriteLine("==ItemExistsDynamicParameters==");
            return null;
        }
        ...
        protected override void GetItem(string path) {
            ...
            var location = GetLocation(path);
            if (location.Exists) {
                WriteItemObject(location, location.AbsolutePath, location.IsItemContainer);
            }
        }
        ...
        protected override object GetItemDynamicParameters(string path) {
            // Console.WriteLine("==GetItemDynamicParameters==");
            return null;
        }
        ...
        protected override object SetItemDynamicParameters(string path, object value) {
            //Console.WriteLine("==SetItemDynamicParameters==");
            return null;
        }
        ...
        protected override object ClearItemDynamicParameters(string path) {
            // Console.WriteLine("==ClearItemDynamicParameters==");
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==InvokeDefaultAction==");
#endif
        }
        ...
        protected override object InvokeDefaultActionDynamicParameters(string path) {
            // Console.WriteLine("==InvokeDefaultActionDynamicParameters==");
            return null;
        }
...
#if TRACE_FINISHED
            Console.WriteLine("==GetChildItems [{0}],{1}==", path, recurse);
#endif
            var location = GetLocation(path);
            if (location.Exists) {
                foreach (var each in location.GetDirectories(recurse)) {
                    WriteItemObject(each, each.AbsolutePath, true);
                }
                foreach (var each in location.GetFiles(recurse)) {
                    WriteItemObject(each, each.AbsolutePath, false);
                }
            }
            // WriteItemObject( item, path, isContainer );
            //
        ...
        protected override object GetChildItemsDynamicParameters(string path, bool recurse) {
            // Console.WriteLine("==GetChildItemsDynamicParameters==");
            return null;
        }
        ...
        protected override void GetChildNames(string path, ReturnContainers returnAllContainers) {
            ...
            var location = GetLocation(path);
            if (location.Exists) {
                foreach (var l in location.GetDirectories(false).Union(location.GetFiles(false))) {
                    WriteItemObject(l.Name, l.AbsolutePath, l.IsItemContainer);
                }
            }
        ...
        protected override object GetChildNamesDynamicParameters(string path) {
            //Console.WriteLine("==GetChildNamesDynamicParameters==");
            return null;
        }
        ...
        protected override object RenameItemDynamicParameters(string path, string newName) {
            // Console.WriteLine("==RenameItemDynamicParameters==");
            return null;
        }
        ...
        protected override object NewItemDynamicParameters(string path, string type, object newItemValue) {
            //Console.WriteLine("==NewItemDynamicParameters==");
            return null;
        }
        ...
        protected override object RemoveItemDynamicParameters(string path, bool recurse) {
            //Console.WriteLine("==RemoveItemDynamicParameters==");
            return null;
        }
        ...
        protected override object CopyItemDynamicParameters(string path, string destination, bool recurse) {
            //Console.WriteLine("==CopyItemDyamicParameters==");
            return null;
        }
        ...
        protected override string MakePath(string parent, string child) {
            return _makePathCache.GetOrAdd(new Tuple<string, string>(parent, child), () => base.MakePath(parent, child));
        }
        ...
        protected override string GetParentPath(string path, string root) {
            return _getParentPathCache.GetOrAdd(new Tuple<string, string>(path, root), () => base.GetParentPath(path, root));
        }
        ...
        protected override string GetChildName(string path) {
            return _getChildNameCache.GetOrAdd(path, () => gcn(path));
        }

        // GetChildName

        protected string gcn(string path) {
            if (string.IsNullOrEmpty(path)) {
                throw new ClrPlusException("path is null");
            }

            path = path.Replace('/', '\\').TrimEnd(new[] {
                '\\'
            });

            int startIndex = path.LastIndexOf('\\');
            return startIndex == -1 ? path : path.Substring(startIndex + 1);
        }
...
#if TRACE_FINISHED
            Console.WriteLine("==IsItemContainer [{0}]==",path);
#endif
            var location = GetLocation(path);
            return location.IsItemContainer;
        }
        ...
        protected override object MoveItemDynamicParameters(string path, string destination) {
            // Console.WriteLine("==MoveItemDynamicParameters==");
            return null;
        }
        ...
        public IContentReader GetContentReader(string path) {
            var location = GetLocation(path);
            if (location.Exists) {
                return location.GetContentReader();
            }
            return null;
        }
        ...
        public IContentWriter GetContentWriter(string path) {
            var location = GetLocation(path);
            return location.GetContentWriter();
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==ClearContent==");
            var location = GetLocation(path);
            if (location.Exists) {
                location.ClearContent();
            }
#endif
        ...
        public object ClearContentDynamicParameters(string path) {
            //Console.WriteLine("==ClearContentDynamicParameters==");
            return null;
        }
        ...
        public object GetPropertyDynamicParameters(string path, Collection<string> providerSpecificPickList) {
            //Console.WriteLine("==GetPropertyDynamicParameters==");
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==SetPropertyDynamicParameters==");
#endif
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==ClearProperty==");
#endif
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==ClearPropertyDynamicParameters==");
#endif
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==CopyProperty==");
#endif
        ...
        public object CopyPropertyDynamicParameters(string sourcePath, string sourceProperty, string destinationPath, string destinationProperty) {
            // Console.WriteLine("==CopyPropertyDynamicParameters==");
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==MoveProperty==");
#endif
        ...
        public object MovePropertyDynamicParameters(string sourcePath, string sourceProperty, string destinationPath, string destinationProperty) {
            //Console.WriteLine("==MovePropertyDynamicParameters==");
            return null;
        }
        ...
        public object NewPropertyDynamicParameters(string path, string propertyName, string type, object value) {
            //Console.WriteLine("==NewPropertyDynamicParameters==");
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==RemoveProperty==");
#endif
        ...
        public object RemovePropertyDynamicParameters(string path, string propertyName) {
            // Console.WriteLine("==RemovePropertyDynamicParameters==");
            return null;
        }
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==RenameProperty==");
#endif
        ...
        public object RenamePropertyDynamicParameters(string path, string sourceProperty, string destinationProperty) {
            //Console.WriteLine("==RenamePropertyDynamicParameters==");
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==GetSecurityDescriptor==");
#endif
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==SetSecurityDescriptor==");
#endif
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==NewSecurityDescriptorFromPath==");
#endif
...
#if TRACE_NOT_FINISHED
            Console.WriteLine("==NewSecurityDescriptorOfType==");
#endif
            ObjectSecurity sd = null;
