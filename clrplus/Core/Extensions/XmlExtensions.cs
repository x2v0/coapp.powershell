...
namespace ClrPlus.Core.Extensions {
    ...
    public static class XmlExtensions {
        ...
        /// </summary>
        private static readonly IDictionary<string, XmlDocument> _docCache = new XDictionary<string, XmlDocument>();
        ...
        public static XmlDocument XmlDoc(this string xmlDoc) {
            if (_docCache.ContainsKey(xmlDoc)) {
                return _docCache[xmlDoc];
            }

            var doc = new XmlDocument();
            doc.LoadXml(xmlDoc);
            _docCache.Add(xmlDoc, doc);
            return doc;
        }

#if TODO
    // mark for delete? I don't think we use this.
